...
export async function getSessionUser(db: D1DB, token: string | null): Promise<User | null> {
	...
	if (!row) return null;
	const user = await db.prepare("SELECT id, email, name, provider FROM users WHERE id = ?").bind(row.user_id).first<User>();
	return user;
}

export async function getOrCreateDemoUser(db: D1DB, email: string, name: string): Promise<User> {
	const existing = await db.prepare("SELECT id, email, name, provider FROM users WHERE provider = 'demo' AND email = ?").bind(email).first<User>();
	if (existing) return existing;
	const id = `user-${crypto.randomUUID()}`;
	await db.prepare("INSERT INTO users (id, email, name, provider, provider_id) VALUES (?, ?, ?, 'demo', ?)").bind(id, email, name, email).run();
...
export async function getPosts(db: D1DB, currentUserId: string | null): Promise<Post[]> {
	const rows = await db
       ...
       LIMIT 100`
		...
		.all<{ id: string; author_id: string; content: string; created_at: string; author_name: string }>();
	const posts: Post[] = [];
	for (const row of rows.results ?? []) {
		const likeRows = await db.prepare("SELECT user_id FROM likes WHERE post_id = ?").bind(row.id).all<{ user_id: string }>();
		...
		posts.push({
			...
			likedByMe: currentUserId ? (likeRows.results?.some((r) => r.user_id === currentUserId) ?? false) : false,
			comments: (commentRows.results ?? []).map((c) => ({ id: c.id, authorName: c.author_name, content: c.content, createdAt: c.created_at })),
		});
	}
	return posts;
}
...
export async function createPost(db: D1DB, authorId: string, content: string): Promise<Post> {
	...
	const user = await db.prepare("SELECT name FROM users WHERE id = ?").bind(authorId).first<{ name: string }>();
	await db.prepare("INSERT INTO posts (id, author_id, content) VALUES (?, ?, ?)").bind(id, authorId, content).run();
	return {
		id,
		authorId,
...
export async function toggleLike(db: D1DB, postId: string, userId: string): Promise<{ likes: number; likedByMe: boolean }> {
	const existing = await db.prepare("SELECT 1 FROM likes WHERE post_id = ? AND user_id = ?").bind(postId, userId).first();
	if (existing) {
		await db.prepare("DELETE FROM likes WHERE post_id = ? AND user_id = ?").bind(postId, userId).run();
		const count = await db.prepare("SELECT COUNT(*) as c FROM likes WHERE post_id = ?").bind(postId).first<{ c: number }>();
...
export async function addComment(db: D1DB, postId: string, authorId: string, content: string) {
	...
	const user = await db.prepare("SELECT name FROM users WHERE id = ?").bind(authorId).first<{ name: string }>();
	await db.prepare("INSERT INTO comments (id, post_id, author_id, content) VALUES (?, ?, ?, ?)").bind(id, postId, authorId, content).run();
...
export async function getFriendStatus(db: D1DB, currentUserId: string, otherUserId: string): Promise<FriendStatus> {
...
export async function sendFriendRequest(db: D1DB, fromId: string, toId: string): Promise<void> {
	...
	await db
		.prepare("INSERT OR IGNORE INTO friend_requests (from_id, to_id, status) VALUES (?, ?, 'pending')")
		.bind(fromId, toId)
		.run();
}
...
export async function rejectFriendRequest(db: D1DB, fromId: string, toId: string): Promise<void> {
	await db.prepare("DELETE FROM friend_requests WHERE from_id = ? AND to_id = ?").bind(fromId, toId).run();
}

// --- Messages ---
export type Message = { id: string; senderId: string; senderName: string; content: string; createdAt: string; readAt: string | null };

export async function getConversations(db: D1DB, userId: string): Promise<{ user: User; lastMessage: string; lastAt: string; unread: number }[]> {
	const all = await db
		.prepare(
       ...
       ORDER BY created_at DESC`
		...
		.bind(userId, userId)
		.all<{ id: string; sender_id: string; receiver_id: string; content: string; created_at: string; read_at: string | null }>();
	...
	for (const otherId of Object.keys(byOther)) {
		const u = await db.prepare("SELECT id, email, name, provider FROM users WHERE id = ?").bind(otherId).first<User>();
...
export async function getMessages(db: D1DB, userId: string, otherUserId: string, limit = 50): Promise<Message[]> {
	const rows = await db
		.prepare(
       ...
       ORDER BY m.created_at DESC LIMIT ?`
		...
		.all<{ id: string; sender_id: string; content: string; created_at: string; read_at: string | null; sender_name: string }>();
	const list = (rows.results ?? []).reverse();
	await db
		.prepare("UPDATE messages SET read_at = datetime('now') WHERE receiver_id = ? AND sender_id = ? AND read_at IS NULL")
...
export async function sendMessage(db: D1DB, senderId: string, receiverId: string, content: string): Promise<Message> {
	const id = `msg-${crypto.randomUUID()}`;
	const user = await db.prepare("SELECT name FROM users WHERE id = ?").bind(senderId).first<{ name: string }>();
	await db
...
export async function getQueuesForCourts(db: D1DB, courtIds: string[]): Promise<Record<string, QueueEntry[]>> {
...
export async function joinCourtQueue(db: D1DB, courtId: string, userId: string): Promise<void> {
	await db
		.prepare("INSERT OR IGNORE INTO court_queue (court_id, user_id, created_at) VALUES (?, ?, datetime('now'))")
		.bind(courtId, userId)
		.run();
	...
	if (admins.length === 0) {
		await db
			.prepare("INSERT OR IGNORE INTO court_admins (court_id, user_id, created_at) VALUES (?, ?, datetime('now'))")
			.bind(courtId, userId)
			.run();
	}
}
...
export async function leaveCourtQueue(db: D1DB, courtId: string, userId: string): Promise<void> {
	await db.prepare("DELETE FROM court_queue WHERE court_id = ? AND user_id = ?").bind(courtId, userId).run();
}
...
export async function getCodesForCourts(db: D1DB, courtIds: string[]): Promise<Record<string, string>> {
...
export async function getAdminsForCourts(db: D1DB, courtIds: string[]): Promise<Record<string, CourtAdminEntry[]>> {
...
export async function removeCourtAdmin(db: D1DB, courtId: string, userId: string): Promise<void> {
	await db.prepare("DELETE FROM court_admins WHERE court_id = ? AND user_id = ?").bind(courtId, userId).run();
}

// --- Tournaments: single-elimination, admin creates and chooses who progresses ---
export type Tournament = { id: string; name: string; adminId: string; adminName: string; status: string; createdAt: string };
...
export async function createTournament(db: D1DB, name: string, adminId: string): Promise<string> {
	...
	await db.prepare("INSERT INTO tournaments (id, name, admin_id, status) VALUES (?, ?, ?, 'draft')").bind(id, name, adminId).run();
	return id;
}
...
export async function getTournaments(db: D1DB, limit = 50): Promise<Tournament[]> {
	const rows = await db
       ...
       ORDER BY t.created_at DESC LIMIT ?`
		...
		.bind(limit)
		.all<{ id: string; name: string; admin_id: string; status: string; created_at: string; admin_name: string }>();
...
export async function getTournament(db: D1DB, id: string): Promise<Tournament | null> {
	const row = await db
       ...
       WHERE t.id = ?`
		...
		.bind(id)
		.first<{ id: string; name: string; admin_id: string; status: string; created_at: string; admin_name: string }>();
	if (!row) return null;
	return { id: row.id, name: row.name, adminId: row.admin_id, adminName: row.admin_name, status: row.status, createdAt: row.created_at };
}

export async function isTournamentAdmin(db: D1DB, tournamentId: string, userId: string): Promise<boolean> {
	const row = await db
...
export async function getTournamentParticipants(db: D1DB, tournamentId: string): Promise<TournamentParticipant[]> {
	const rows = await db
		.prepare(
	...
	return (rows.results ?? []).map((r) => ({ userId: r.user_id, userName: r.user_name, seed: r.seed }));
}

export async function addTournamentParticipant(db: D1DB, tournamentId: string, userId: string): Promise<void> {
	...
	await db
		.prepare("INSERT OR IGNORE INTO tournament_participants (tournament_id, user_id, seed) VALUES (?, ?, ?)")
...
export async function startTournament(db: D1DB, tournamentId: string): Promise<{ error?: string }> {
	...
	if (Math.floor(log2) !== log2) return { error: "Participants must be a power of 2 (2, 4, 8, 16)" };
	await db.prepare("UPDATE tournaments SET status = 'in_progress' WHERE id = ?").bind(tournamentId).run();
	const numRounds = Math.ceil(Math.log2(n));
	const round1Matches = n / 2;
	...
	for (let round = 1; round <= numRounds; round++) {
		...
		for (let order = 1; order <= matchesInRound; order++) {
			...
			const nextMatchId = round < numRounds ? null : null;
			const nextSlot = round < numRounds ? null : null;
			await db
				.prepare(
	...
	for (let round = 1; round < numRounds; round++) {
		...
		for (let order = 1; order <= matchesInRound; order++) {
			...
			await db.prepare("UPDATE bracket_matches SET next_match_id = ?, next_slot = ? WHERE id = ?").bind(nextMatchId, nextSlot, matchId).run();
		}
	}
	return {};
}
...
export async function getBracketMatches(db: D1DB, tournamentId: string): Promise<BracketMatch[]> {
	...
	for (const r of rows.results ?? []) {
		...
		if (r.player1_id) p1Name = (await db.prepare("SELECT name FROM users WHERE id = ?").bind(r.player1_id).first<{ name: string }>())?.name ?? null;
		if (r.player2_id) p2Name = (await db.prepare("SELECT name FROM users WHERE id = ?").bind(r.player2_id).first<{ name: string }>())?.name ?? null;
		if (r.winner_id) winnerName = (await db.prepare("SELECT name FROM users WHERE id = ?").bind(r.winner_id).first<{ name: string }>())?.name ?? null;
		out.push({
			id: r.id,
...
export async function setMatchWinner(db: D1DB, matchId: string, winnerId: string): Promise<{ error?: string }> {
	const match = await db
		.prepare("SELECT id, tournament_id, player1_id, player2_id, next_match_id, next_slot FROM bracket_matches WHERE id = ?")
		.bind(matchId)
		.first<{ id: string; tournament_id: string; player1_id: string | null; player2_id: string | null; next_match_id: string | null; next_slot: number | null }>();
	if (!match) return { error: "Match not found" };
	if (winnerId !== match.player1_id && winnerId !== match.player2_id) return { error: "Winner must be a player in this match" };
	await db.prepare("UPDATE bracket_matches SET winner_id = ? WHERE id = ?").bind(winnerId, matchId).run();
	if (match.next_match_id && match.next_slot) {
		if (match.next_slot === 1) {
			await db.prepare("UPDATE bracket_matches SET player1_id = ? WHERE id = ?").bind(winnerId, match.next_match_id).run();
		} else {
			await db.prepare("UPDATE bracket_matches SET player2_id = ? WHERE id = ?").bind(winnerId, match.next_match_id).run();
		}
	}
	return {};
}
...
export async function deleteCoachingListing(db: D1DB, listingId: string, userId: string): Promise<{ error?: string }> {
	const row = await db
